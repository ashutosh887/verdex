...
micro-F1 **0.745** · macro-F1 **0.459** · Hamming loss **0.054** · exact-match **0.600**

## 3. Abstention & calibration

- **AURC** (area under risk–coverage) **0.1532** — lower is better; does it abstain on the cases it would otherwise get wrong?
- **ECE** (10-bin) **0.195** · **Brier** **0.2235**.
...
- **user_005** (conf 0.9): status gold=`contradicted`/pred=`not_enough_information`, issue `scratch`/`unknown`, part `rear_bumper`/`rear_bumper`, sev `low`/`unknown`; flags gold=`claim_mismatch;user_history_risk;manual_review_required` pred=`user_history_risk;manual_review_required`.
...
- **user_012** (conf 0.95): status gold=`supported`/pred=`not_enough_information`, issue `dent`/`unknown`, part `corner`/`corner`, sev `low`/`unknown`; flags gold=`none` pred=`damage_not_visible;possible_manipulation;manual_review_required`.
...
- **user_032** (conf 0.9): status gold=`not_enough_information`/pred=`not_enough_information`, issue `unknown`/`unknown`, part `contents`/`box`, sev `unknown`/`unknown`; flags gold=`cropped_or_obstructed;damage_not_visible;manual_review_required` pred=`damage_not_visible;manual_review_required`.
...
- Model calls (sample run): **70** (21 billed, 49 cached).
- Tokens: **46,654** in / **2,332** out.
- Images processed: **29**.
- Sample-set est. cost: **$0.14** (≈ $0.0070/claim).
- Extrapolated full test set (44 rows): **≈ $0.31** (pricing assumptions in `config.yaml`; verify vs current OpenAI pricing).
- Wall-clock: **66.3s** for 20 claims (serial).
